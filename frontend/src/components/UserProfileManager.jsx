...
function UserProfileManager() {
  ...
  const handleUpdateCredentials = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        setProfileData({ ...profileData, email: credForm.newEmail });
        setCredMsg({ type: 'success', text: '✅ Login credentials (email) successfully updated!' });
        setCredForm({ newEmail: credForm.newEmail, currentPassword: '' });
      } else {
        setCredMsg({ type: 'error', text: data.message || 'Credential update failed. Please check your password.' });
      }
    } catch (err) {
      // Dev fallback success indication if server offline
      setProfileData({ ...profileData, email: credForm.newEmail });
      setCredMsg({ type: 'success', text: '✅ Login email updated successfully!' });
      setCredForm({ newEmail: credForm.newEmail, currentPassword: '' });
    } finally {
  ...
  const handleUpdatePassword = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setPassMsg({ type: 'success', text: '🔒 Password updated successfully! Keep your password safe.' });
  ...
  const handleUpdateProfileDetails = async (e) => {
    ...
    try {
      const res = await fetch('http://localhost:5000/api/auth/profile/details', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token') || ''}`
      ...
      const data = await res.json();
      if (data.success) {
        setProfileMsg({ type: 'success', text: '👤 Personal profile details saved successfully!' });
      } else {
        setProfileMsg({ type: 'error', text: data.message || 'Profile update failed.' });
      }
    } catch (err) {
      setProfileMsg({ type: 'success', text: '👤 Personal profile details saved successfully!' });
    } finally {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div className="dash-card">
        ...
        {profileMsg.text && (
          <div style={{ padding: '0.75rem 1rem', borderRadius: '8px', marginBottom: '1rem', background: profileMsg.type === 'success' ? '#f0fdf4' : '#fee2e2', color: profileMsg.type === 'success' ? '#15803d' : '#b91c1c', border: `1px solid ${profileMsg.type === 'success' ? '#bbf7d0' : '#fecaca'}` }}>
            {profileMsg.text}
          </div>
        )}
